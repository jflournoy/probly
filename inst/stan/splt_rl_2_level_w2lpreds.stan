data {
    ...
    int<lower=1> J; //number if subject-level predictors _including intercept_.
    ...
    matrix[N, J] u; //N-level predictors. for adding, e.g., age and puberty _INCLUDE INTERCEPT_
...
parameters {
    ...
    matrix[J,K] mu_delta_xi; //overall mean for each condition
    matrix[J,K] mu_delta_ep;
    matrix[J,K] mu_delta_b;
    matrix[J,K] mu_delta_rho;
}

...
```{r estimate_rl_from_sim, eval=T, message=F}
```

The model as described above was estimated using RStan [version `r packageVersion('rstan')`; @standevelopmentteam2018], sampling from 4 chains with 1000 warmup iterations and 500 sampling iterations per chain.   


```{r checking_fit_mu, fig.width=8, fig.height=5, fig.cap="Capturing data-generating population means"}
...
ggplot2::ggplot(
   ...
   ggplot2::aes(y = mean, x = value)) + 
    ...
    ggplot2::theme_minimal() +
    ggplot2::facet_grid(id ~ pop_parameter, scales = 'free', labeller = 'label_parsed') +

---
title: "Association of Age and Puberty with Learning Parameters"
...
multi_plot <- probly::plot_splt_sim_behavior(
    splt_df = splt,
    press_right = list_of_pright_pred_mats[sample(1:400, size = 100)])
multi_plot + 
...
```{r simdatcoverage, fig.width=8, fig.height=5, fig.cap='(ref:simdatcoverage)'}
...
bayesplot::mcmc_areas_ridges(pnorm(true_mu_ep))
bayesplot::mcmc_areas_ridges(true_beta_ep, adjust = .5)
bayesplot::mcmc_areas_ridges(true_mu_rho) + ggplot2::coord_cartesian(xlim = c(-10, 10))
bayesplot::mcmc_areas_ridges(log(true_beta_rho), adjust = 2) + ggplot2::coord_cartesian(xlim = c(-10, 10))
bayesplot::mcmc_areas_ridges(pnorm(true_mu_xi))
bayesplot::mcmc_areas_ridges(true_beta_xi, adjust = .5)
```

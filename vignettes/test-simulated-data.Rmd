...
```{r}
idxs <- sample(1:400, size = 100)
list_of_pright_pred_mats <- lapply(idxs, function(i) matrix(pright_pred_samps[i,], nrow = N))
...
```{r simdata, fig.width=15, fig.height=12, fig.cap='(ref:simdat)'}
multi_plot + 
...
## Simulating more iterations

Although the above results are promising, it is important to ensure that this is not a fluke.
Unfortunately, it is apparent from running more models on other simulated data that many converge very slowly and all show post-warm-up divergent iterations.
...
stan_sim_lists <- readRDS('/data/jflournoy/split/probly/sim_test_100_iter.RDS')

stem(unlist(lapply(stan_sim_lists, function(reslist){
    sum(unlist(lapply(reslist$sampler_params, function(samplist){
        sum(samplist[51:100, 'divergent__'])
    })))
})), scale = 50)

stan_sim_lists[[1]]$fit_summary$summary['mu_delta_xi[1]', 'mean']
```

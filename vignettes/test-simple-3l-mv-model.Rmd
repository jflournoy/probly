...
The model for our simulated data is:
...
stan_fit_fn <- '/data/jflournoy/split/probly/stan_test_fit_c_l3.RDS'
if(!file.exists()){
    plan(multiprocess)
    stan_m <- rstan::stan_model(file = '../exec/splt_rl_mvnorm_test_c_l3.stan')
    stan_optim <- rstan::optimizing(stan_m,
                                    data = data_for_stan)
    round(stan_optim$par[grep('delta', names(stan_optim$par))],3)
    stan_fit_test_f <- future({rstan::stan(file = '../exec/splt_rl_mvnorm_test_c_l3.stan',
                                           data = data_for_stan, chains = 4, iter = 1500,
                                           warmup = 1000, cores = 4, open_progress = T)})
    saveRDS(stan_fit_test, stan_fit_fn)
} else {
    stan_fit_test <- readRDS(stan_fit_fn)
}
stan_fit_fn.orig <- '/data/jflournoy/split/probly/stan_test_fit.RDS'
stan_fit_fn.repar <- '/data/jflournoy/split/probly/stan_test_fit2.RDS'
stan_fit_test.orig <- readRDS(stan_fit_fn.orig)
stan_fit_test.repar <- readRDS(stan_fit_fn.repar)

par_names <- grep('(delta|beta)', names(stan_fit_test), value = T)
stan_fit_extract <- rstan::extract(stan_fit_test, pars = par_names)
bayesplot::mcmc_dens(as.array(stan_fit_test), regex_pars = 'mu_delta\\[\\d\\]')
bayesplot::mcmc_trace(as.array(stan_fit_test), regex_pars = 'mu_delta\\[\\d\\]')
stan_fit_means <- lapply(stan_fit_extract, mean)
print(
    ggplot2::qplot(as.vector(beta_j),
                   as.numeric(
                       stan_fit_means[grep('beta\\[', names(stan_fit_means))])) +
        geom_abline(intercept = 0, slope = 1))
print(
    ggplot2::qplot(as.vector(gamma_mu),
                   as.numeric(
                       stan_fit_means[grep('delta\\[\\d,', names(stan_fit_means))])) +
        geom_abline(intercept = 0, slope = 1))
rbind(stan = stan_fit_means[grep('mu_delta', names(stan_fit_means))],
      raw_p = arm::logit(unlist(dplyr::summarize(dplyr::group_by(dd, condition), mean = mean(y))[,2])),
      group_p = apply(gamma_mu, 2, mean))
stan_fit_means$sigma_delta

posterior <- as.array(stan_fit_test)
bayesplot::mcmc_trace(posterior, regex_pars = 'delta\\[\\d,\\d\\]')
summary(stan_fit_test, pars = par_names, probs = c(.025, .5, .975))

par_names.o <- grep('(delta|beta)', names(stan_fit_test.orig), value = T)
par_names.r <- grep('(delta|beta)', names(stan_fit_test.repar), value = T)
stan_fit_extract.o <- rstan::extract(stan_fit_test.orig, pars = par_names.o)
stan_fit_extract.r <- rstan::extract(stan_fit_test.repar, pars = par_names.r)
bayesplot::mcmc_dens(as.array(stan_fit_test.orig), regex_pars = 'mu_delta\\[\\d\\]')
bayesplot::mcmc_dens(as.array(stan_fit_test.repar), regex_pars = 'mu_delta\\[\\d\\]')
bayesplot::mcmc_trace(as.array(stan_fit_test.orig), regex_pars = 'mu_delta\\[\\d\\]')
bayesplot::mcmc_trace(as.array(stan_fit_test.repar), regex_pars = 'mu_delta\\[\\d\\]')
stan_fit_means <- lapply(stan_fit_extract, mean)
print(
    ggplot2::qplot(as.vector(beta_j),
                   as.numeric(

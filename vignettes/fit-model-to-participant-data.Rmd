...
```{r}
covsamps_ep <- probly::extract_cor_cov_samps(stanfit_2_level_no_b, par_subscript = 'ep')
covsamps_xi <- probly::extract_cor_cov_samps(stanfit_2_level_no_b, par_subscript = 'xi')
covsamps_rho <- probly::extract_cor_cov_samps(stanfit_2_level_no_b, par_subscript = 'rho')
...
```{r}
par_regex_list <- unlist(lapply(c('ep', 'xi', 'rho'), paste0, '_prm\\[\\d+,', 1:3, '\\]'))
...
var_stuff <- lapply(par_regex_list, function(par_regex){
    if(grepl('(xi|ep)', par_regex)){
        par_samples_trans <-  qnorm(stanfit_2_level_no_b_betas[,grepl(par_regex,
                                                                      dimnames(stanfit_2_level_no_b_betas)[[2]])])
    } else if(grepl('rho', par_regex)){
        par_samples_trans <-  log(stanfit_2_level_no_b_betas[,grepl(par_regex,
                                                                    dimnames(stanfit_2_level_no_b_betas)[[2]])])
    }
    par_means <- apply(par_samples_trans, 2, mean)
    var_means <- var(par_means)
    var_tot <- var(as.numeric(par_samples_trans))
    alist <- list(c(var_means = var_means, var_tot = var_tot, prop_var = var_means/var_tot))
    names(alist) <- sub('(ep|xi|rho).*([1-3]).*', '\\1_\\2', par_regex)
    alist
})

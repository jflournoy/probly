...
lowfi_df_dev <- dplyr::left_join(
    ...
    splt_fsmi_matestat_scored, by = c('id' = 'SID')) %>%
...
p_opt_age_plot <- ggplot2::ggplot(dplyr::filter(lowfi_df_dev_v_bl, condition != 'Hungry/Thirsty'),
                ggplot2::aes(x = age, y = bin_5_to_end, linetype = condition)) + 
    ggplot2::geom_point() + 
    ggplot2::geom_smooth(color = 'black',
                         method = 'gam', formula = y ~ s(x, fx = T, k = 5),
                         size = .5, alpha =  .2) + 
    ggplot2::labs(x = 'Age', y = 'Probability of making optimal choice\nover Hungry/Thirsty baseline')
    
p_opt_pds_plot <- ggplot2::ggplot(dplyr::filter(lowfi_df_dev_v_bl, condition != 'Hungry/Thirsty'),
    ...
    ggplot2::facet_grid(~factor(gender, levels = c(0,1), labels = c('male', 'female')),
                        scales = 'free_x') +
    ggplot2::scale_color_manual(name = 'Gender', breaks = c(0, 1), values = c('#1f78b4', '#33a02c'),
                                labels = c('Male', 'Female'))

p_opt_mate_plot <- ggplot2::ggplot(dplyr::filter(lowfi_df_dev_v_bl, condition != 'Hungry/Thirsty',
                              grepl('yads', sample)),
                ggplot2::aes(x = fsmi_mate, y = bin_5_to_end, linetype = condition)) + 
    ggplot2::geom_point() + 
    ggplot2::geom_smooth(color = 'black',
                         method = 'lm', formula = y ~ x,
                         size = .5, alpha = .2) + 
    ggplot2::labs(x = 'FSMI mate-seeking', y = 'Probability of making optimal choice\nover Hungry/Thirsty baseline')

p_opt_stat_plot <- ggplot2::ggplot(dplyr::filter(lowfi_df_dev_v_bl, condition != 'Hungry/Thirsty',
                              grepl('yads', sample)),
                ggplot2::aes(x = fsmi_stat, y = bin_5_to_end, linetype = condition)) + 
    ggplot2::geom_point() + 
    ggplot2::geom_smooth(color = 'black',
                         method = 'lm', formula = y ~ x,
                         size = .5, alpha = .2) + 
    ggplot2::labs(x = 'FSMI status', y = 'Probability of making optimal choice\nover Hungry/Thirsty baseline')
...
betas_rho_means <- lapply(list(
    ...
    function(stanmat){
        apply(stanmat[,betanames[(2*3*313+1):(3*3*313)]], 
              2, function(j) median(log(j)))
    })

model_par_corpairs_plot_xi <- GGally::ggpairs(as.data.frame(betas_xi_means), title = expression(xi))
model_par_corpairs_plot_epsilon <- GGally::ggpairs(as.data.frame(betas_ep_means), title = expression(epsilon))
model_par_corpairs_plot_rho <- GGally::ggpairs(as.data.frame(betas_rho_means), title = expression(rho))
model_par_corpairs_plot_xi_and_rho <- GGally::ggpairs(as.data.frame(c(betas_rho_means, betas_xi_means)), title = 'xi rho')

model_par_corpairs_plot_xi
model_par_corpairs_plot_epsilon
model_par_corpairs_plot_rho
model_par_corpairs_plot_xi_and_rho
```

```{r}

ht_cond_for_contrast <- 
    stanfit_2_level_no_b_betas[,
                               c(rep((313*0*3+313*0+1):(313*0*3+313*1),3),
                                 rep((313*1*3+313*0+1):(313*1*3+313*1),3),
                                 rep((313*2*3+313*0+1):(313*2*3+313*1),3))]
stanfit_2_level_no_b_betas_contrasted <- stanfit_2_level_no_b_betas - ht_cond_for_contrast

stanfit_2_level_no_b_betas_df <- do.call(
    rbind,
    apply(
        cbind(stanfit_2_level_no_b_betas, stanfit_2_level_no_b_betas_contrasted),
        2,
        function(row) data.frame(mean = mean(row), median = median(row), sd = sd(row))))
stanfit_2_level_no_b_betas_df$par <- dimnames(stanfit_2_level_no_b_betas)[[2]]   
stanfit_2_level_no_b_betas_df$contrasted <- rep(c(F,T), each = dim(stanfit_2_level_no_b_betas)[[2]])

splt_rl_betas <- stanfit_2_level_no_b_betas_df %>%
    tidyr::extract(par, c('parameter', 'idx', 'condnum'), 'beta_(.*)_prm\\[(\\d+),([1-3])\\]') %>%
    dplyr::left_join(mutate(distinct(splt_orig, id), idx = as.character(1:n()))) %>%
...
confidence_parameter_plot <- ggplot2::ggplot(splt_rl_betas_sum,
    ...
    ggplot2::geom_smooth(alpha = .5, method = 'gam', formula = y ~ s(x, fx = T, k = 3),
                         color = 'black', size = .5) + 
...
which_model <- 'L3_no_b'
splt_learning_betas <- data.frame(par = names(betas_ep_means[[which_model]]),
           ep_mean = betas_ep_means[[which_model]],
           rho_mean = betas_rho_means[[which_model]],
           xi_mean = betas_xi_means[[which_model]]) %>%
    mutate(condition_num = as.numeric(sub('.*\\[\\d+,(\\d)\\]', '\\1', par)),
           ...
           eprho = pnorm(ep_mean)*exp(rho_mean)*10) %>%
...
ggplot2::ggplot(lowfi_learning_df,
...
lowfi_learning_pdiffs_df <- group_by(lowfi_learning_df, id) %>% 
    mutate(rho_mean_contrast = exp(rho_mean) - exp(rho_mean[condition == 'Hungry/Thirsty']),
           ep_mean_contrast = pnorm(ep_mean) - pnorm(ep_mean[condition == 'Hungry/Thirsty']),
           xi_mean_contrast = pnorm(xi_mean) - pnorm(xi_mean[condition == 'Hungry/Thirsty']),
           bin5end_diff = pnorm(bin_5_to_end) - pnorm(bin_5_to_end[condition == 'Hungry/Thirsty']))
...
fsmims_ep_test_tau <- cor.test(
    lowfi_learning_pdiffs_df$ep_mean[lowfi_learning_pdiffs_df$condition == 'Dating/Looking'],
    lowfi_learning_pdiffs_df$fsmi_mate[lowfi_learning_pdiffs_df$condition == 'Dating/Looking'],
    method = 'kendall',
    exact = F)

fsmims_ep_test_tau$statistic
fsmims_ep_test_tau$p.value
fsmims_ep_test_tau$r <- c(r = sin(.5 * pi * fsmims_ep_test_tau$estimate))
fsmims_ep_test_tau$r
fsmims_ep_test_tau$d <- c(d = 2*fsmims_ep_test_tau$r / ( 1 - fsmims_ep_test_tau$r^2 )^.5 )
fsmims_ep_test_tau$d

fsmist_ep_test_tau <- cor.test(
    lowfi_learning_pdiffs_df$ep_mean[lowfi_learning_pdiffs_df$condition == 'Popular/Unpopular'],
    lowfi_learning_pdiffs_df$fsmi_stat[lowfi_learning_pdiffs_df$condition == 'Popular/Unpopular'],
    method = 'kendall',
    exact = F)

fsmist_ep_test_tau$statistic
fsmist_ep_test_tau$p.value
fsmist_ep_test_tau$r <- c(r = sin(.5 * pi * fsmist_ep_test_tau$estimate))
fsmist_ep_test_tau$r
fsmist_ep_test_tau$d <- c(d = 2*fsmist_ep_test_tau$r / ( 1 - fsmist_ep_test_tau$r^2 )^.5 )
fsmist_ep_test_tau$d
...
```{r}
munames <- grep('mu', names(stanfit_2_level_no_b), value = T)
stanfitmat <- as.array(stanfit_2_level_no_b)
...
mu_ep_1 <- rstan::extract(stanfit_2_level_no_b, pars = munames[4])[[1]]
mu_ep_2 <- rstan::extract(stanfit_2_level_no_b, pars = munames[5])[[1]]
mu_ep_3 <- rstan::extract(stanfit_2_level_no_b, pars = munames[6])[[1]]
...
mu_rho_1 <- rstan::extract(stanfit_2_level_no_b, pars = munames[7])[[1]]
mu_rho_2 <- rstan::extract(stanfit_2_level_no_b, pars = munames[8])[[1]]
mu_rho_3 <- rstan::extract(stanfit_2_level_no_b, pars = munames[9])[[1]]
...
mu_xi_1 <- rstan::extract(stanfit_2_level_no_b, pars = munames[1])[[1]]
mu_xi_2 <- rstan::extract(stanfit_2_level_no_b, pars = munames[2])[[1]]
mu_xi_3 <- rstan::extract(stanfit_2_level_no_b, pars = munames[3])[[1]]
...
beta_xi_mat <- matrix(
    rep(apply(rstan::extract(stanfit_2_level_no_b, pars = 'mu_delta_xi')[[1]], c(2,3), mean), 
        313), 
    nrow = 313, 
    byrow = T)
beta_b_mat <- matrix(
    ...
    byrow = T)
beta_ep_mat <- matrix(
    rep(apply(rstan::extract(stanfit_2_level_no_b, pars = 'mu_delta_ep')[[1]], c(2,3), mean), 
        313), 
    nrow = 313, 
    byrow = T)
beta_rho_mat <- matrix(
    rep(apply(rstan::extract(stanfit_2_level_no_b, pars = 'mu_delta_rho')[[1]], c(2,3), mean), 
        313), 
    nrow = 313, 
    byrow = T)

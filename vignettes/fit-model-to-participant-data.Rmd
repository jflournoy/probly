...
p_opt_early_pds_plot <- ggplot2::ggplot(
    dplyr::mutate(
        lowfi_df_dev,
        agegroup = ifelse(grepl('yads', sample), 'College students', 'Adolescents')),
    ggplot2::aes(x = PDS_mean_score, y = bin_start_to_4,
                 color = factor(gender, levels = c(0,1), labels = c('male', 'female')))) + 
    ggplot2::geom_smooth(color = '#999999',
                         method = 'gam', formula = y ~ s(x, fx = T, k = 3),
                         size = 2, alpha = .2) + 
...
p_opt_age_plot
p_opt_early_age_plot
p_opt_pds_plot
p_opt_early_pds_plot
p_opt_diff_age_plot
p_opt_diff_pds_plot
p_opt_fsmi_scales
p_opt_ksrq_scales
...
adf_col <- dplyr::filter(lowfi_df_dev, 
                         grepl('yads', sample))
adf_all <- lowfi_df_dev
adf_all_con <- dplyr::filter(lowfi_df_dev_v_bl, condition != 'Hungry/Thirsty')
...
outcomelist <- list('bin_5_to_end', 'bin_start_to_4')
age_table <- mlm_std_beta_table(outcomelist, xstring = 'age', adf_adol = adf_adol, adf_col = adf_col, adf_all = adf_all)
pds_table <- mlm_std_beta_table(outcomelist, xstring = 'PDS_mean_score', adf_adol = adf_adol, adf_col = adf_col, adf_all = adf_all)
age_con_table <- mlm_std_beta_table(outcomelist, xstring = 'age', 
                                adf_adol = adf_adol_con, adf_col = adf_col_con, adf_all = adf_all_con)
...
```{r}
knitr::kable(
    dplyr::select(dplyr::filter(lowfi_regression_table, Parameter == 'bin_5_to_end'), -Parameter),
    col.names = c('Variables', 'Age-group', '$\\beta$', '$\\text{SE}_{\\beta}$', '$t$'),
    digits = 2,
    caption = 'Optimal choices during last half of the run')

knitr::kable(
    dplyr::select(dplyr::filter(lowfi_regression_table, Parameter == 'bin_start_to_4'), -Parameter),
    col.names = c('Variables', 'Age-group', '$\\beta$', '$\\text{SE}_{\\beta}$', '$t$'),
    digits = 2,
    caption = 'Optimal choices during first half of the run')
```
...
summary(value(cond_time_simpl_m_f))
...
```{r}
stanfit_2_level <- readRDS(file.path(data_dir,'splt-looser-rl_2_level_rho5-1549470.RDS'))
stanfit_list <- lapply(
    dir('/data/jflournoy/split/probly/', pattern = '^splt-looser-rl_*', full.names = T),
    function(fname){
...
names(stanfit_list) <- dir('/data/jflournoy/split/probly/', pattern = '^splt-looser-rl_*')
...
If we can trust these LOO estimates (it would be better to do leave-one-observation-out rather than leave-one-participant-out), it looks like the three level structure is probably unnecessary but that the full parameterization is helpful, including treating rho as a categorical dummy (that is, not imposing a linearity constraint on points 0, 1, and 5).
...
beta_pars <- c('epsilon' = 'beta_ep_prm', 'rho' = 'beta_rho_prm', 'rho5' = 'beta_rho_prm5',
               'xi' = 'beta_xi_prm', 'b' = 'beta_b', 'P("right")[paste(t," = T")]' = 'pR_final')
beta_names_list <- lapply(
    beta_pars,
    function(par){
        grep(paste0('^', par, '\\[\\d+,\\d+\\]'),
             names(mean_pars$`splt-looser-rl_2_level_rho5-1549470.RDS`),
             value = T)
    })
...
two_three_level_comparison_plots <- lapply(
    beta_pars[-which(names(beta_pars) == 'rho5')],
    function(par){
        adf <- dplyr::data_frame(
            'two level' = unlist(mean_pars$`splt-looser-rl_2_level-1545567.RDS`[beta_names_list[[par]]]),
            'two level 2rho' = unlist(mean_pars$`splt-looser-rl_2_level_rho5-1549470.RDS`[beta_names_list[[par]]]),
...
Clearly, the three-level model is very similar to the two-level model.

```{r}
one_two_rho_comparison_plots <- c(lapply(
    beta_pars[-which(names(beta_pars) == 'rho5')],
    function(par){
        adf <- dplyr::data_frame(
            'linear rho' = unlist(mean_pars$`splt-looser-rl_2_level-1545567.RDS`[beta_names_list[[par]]]),
            'categorical rho' = unlist(mean_pars$`splt-looser-rl_2_level_rho5-1549470.RDS`[beta_names_list[[par]]]))
        ggplot2::ggplot(
            adf,
            ggplot2::aes(x = `linear rho`, y = `categorical rho`)) + 
            ggplot2::geom_point() + 
            ggplot2::labs(title = parse(text=bquote(.(names(beta_pars)[which(beta_pars == par)]))),
                          x = '', y = '')
    }),
    list({adf <- dplyr::data_frame(
        'linear rho' = unlist(mean_pars$`splt-looser-rl_2_level-1545567.RDS`[beta_names_list[['beta_rho_prm']]]),
        'categorical rho' = unlist(mean_pars$`splt-looser-rl_2_level_rho5-1549470.RDS`[beta_names_list[['beta_rho_prm5']]]))
    ggplot2::ggplot(
        adf,
        ggplot2::aes(x = `linear rho`, y = `categorical rho`)) + 
        ggplot2::geom_point() + 
        ggplot2::labs(title = expression(paste(rho[5])),
                      x = '', y = '')}))

one_two_rho_comparison_plots[[1]] <- one_two_rho_comparison_plots[[1]] + ggplot2::labs(y = 'Categorical rho')
one_two_rho_comparison_plots[[5]] <- one_two_rho_comparison_plots[[5]] + ggplot2::labs(x = 'Linear rho')
```

```{r twothreecorplot, fig.width=5.875, fig.height=4, include=F}
gridExtra::grid.arrange(grobs = one_two_rho_comparison_plots,
                        ncol = 3)
```

Allowing $\rho$ to be categorical chanes parameters more significantly. The correlations for these plots are:

```{r}
par_means_for_corplot_rho5 <- 
    dplyr::data_frame(
        'epsilon["max"]' = qnorm(unlist(
            mean_pars$`splt-looser-rl_2_level-1545567.RDS`[beta_names_list[['beta_ep_prm']]])),
        'epsilon["rho5"]' = qnorm(unlist(
            mean_pars$`splt-looser-rl_2_level_rho5-1549470.RDS`[beta_names_list[['beta_ep_prm']]])),
        'xi["max"]' = qnorm(unlist(
            mean_pars$`splt-looser-rl_2_level-1545567.RDS`[beta_names_list[['beta_xi_prm']]])),
        'xi["rho5"]' = qnorm(unlist(
            mean_pars$`splt-looser-rl_2_level_rho5-1549470.RDS`[beta_names_list[['beta_xi_prm']]])),
        'rho["max"]' = log(unlist(
            mean_pars$`splt-looser-rl_2_level-1545567.RDS`[beta_names_list[['beta_rho_prm']]])),
        'rho["rho5"]' = log(unlist(
            mean_pars$`splt-looser-rl_2_level_rho5-1549470.RDS`[beta_names_list[['beta_rho_prm']]])),
        '{rho^{5}}["max"]' = log(unlist(
            mean_pars$`splt-looser-rl_2_level-1545567.RDS`[beta_names_list[['beta_rho_prm']]])),
        '{rho^{5}}["rho5"]' = log(unlist(
            mean_pars$`splt-looser-rl_2_level_rho5-1549470.RDS`[beta_names_list[['beta_rho_prm5']]])),
        'b["max"]' = unlist(
            mean_pars$`splt-looser-rl_2_level-1545567.RDS`[beta_names_list[['beta_b']]]),
        'b["rho5"]' = unlist(
            mean_pars$`splt-looser-rl_2_level_rho5-1549470.RDS`[beta_names_list[['beta_b']]])
        )
par_means_for_corplot_rho5_prfinal <- 
    dplyr::data_frame(
        'pR_final["max"]' = unlist(
            mean_pars$`splt-looser-rl_2_level-1545567.RDS`[beta_names_list[['pR_final']]]),
        'pR_final["rho5"]' = unlist(
            mean_pars$`splt-looser-rl_2_level_rho5-1549470.RDS`[beta_names_list[['pR_final']]])
    )
psych::corr.test(dplyr::select(par_means_for_corplot_rho5, contains('eps')), method = 'spearman')
psych::corr.test(dplyr::select(par_means_for_corplot_rho5, contains('xi')), method = 'spearman')
psych::corr.test(dplyr::select(par_means_for_corplot_rho5, contains('rho[')), method = 'spearman')
psych::corr.test(dplyr::select(par_means_for_corplot_rho5, contains('rho^{5}}[')), method = 'spearman')
psych::corr.test(dplyr::select(par_means_for_corplot_rho5, contains('b')), method = 'spearman')
psych::corr.test(dplyr::select(par_means_for_corplot_rho5_prfinal, contains('pR')), method = 'spearman')
```

We can also examine the correlation of parameters as we drop the terms $b$ and $\rho$ from the continuous-$\rho$ models.
...
par_mean_cor_plot_max2l_2rho <- GGally::ggpairs(
    dplyr::select(par_means_for_corplot_rho5, contains('rho5'))[c(2,3,4,1,5)], 
    labeller = ggplot2::label_parsed, 
...
print(par_mean_cor_plot_max2l_2rho)
...
```{r}
#get just betas 
stanfit_2_level_betas <- stanfit_list$`splt-looser-rl_2_level_rho5-1549470.RDS`$parameters[unlist(beta_names_list[1:5])]
...
splt_rl_betas_sum <- splt_rl_betas %>%
    ...
    dplyr::mutate(mean_par = case_when(parameter == 'ep_prm' ~ qnorm(mean_par),
                                       ...
                                       parameter == 'rho_prm5' ~ log(mean_par)),
...
print(
    psych::corr.p(
        psych::corr.test(
            filter(splt_rl_betas_sum, parameter == 'rho_prm5', block == 8)[,c('mean_par', 'confidence')],
            method = 'spearman')$r,
        n = 300, alpha = .005),
    short = F)
print(
...
```{r}
mean_pars_model <- mean_pars$`splt-looser-rl_2_level-1545567.RDS`
...
param_names <- c('ep_prm', 'xi_prm', 'rho_prm', 'b')
...
contrast_var_prop <- lapply(param_names, function(apar){
    beta_names_3 <- grep(paste0(apar, '\\[\\d+,3\\]'), names(stanfit_2_level_betas), value = T)
    beta_names_2 <- grep(paste0(apar, '\\[\\d+,2\\]'), names(stanfit_2_level_betas), value = T)
    beta_names_1 <- grep(paste0(apar, '\\[\\d+,1\\]'), names(stanfit_2_level_betas), value = T)
    
    betas_31 <- lapply(seq_along(beta_names_3), function(i){
        somesamples <- stanfit_2_level_betas[[beta_names_3[i]]] - 
            stanfit_2_level_betas[[beta_names_1[i]]]
    })
    betas_21 <- lapply(seq_along(beta_names_3), function(i){
        somesamples <- stanfit_2_level_betas[[beta_names_2[i]]] - 
            stanfit_2_level_betas[[beta_names_1[i]]]
    })
    
    prmmeanvar31 <- var(unlist(lapply(betas_31, mean)))
    allvar31 <- var(unlist(betas_31))
    
    prmmeanvar21 <- var(unlist(lapply(betas_21, mean)))
    allvar21 <- var(unlist(betas_21))
    
    data.frame(par = apar, `31` = prmmeanvar31/allvar31, `21` = prmmeanvar21/allvar21)
})

knitr::kable(t(as.data.frame(var_stuff)), row.names = TRUE, digits = 2)
knitr::kable(t(as.data.frame(contrast_var_prop)), row.names = TRUE, digits = 2)
```
...
```{r fig.width = 5.875, fig.height=4}
ggplot2::ggplot(lowfi_df_dev,
                ggplot2::aes(x = factor(condition, levels = condition_labels), y = bin_5_to_end)) + 
    ggplot2::geom_boxplot(width = .5) + 
    ggplot2::facet_grid(~sample) + 
    ggplot2::theme(axis.text.x = element_text(angle = 360 - 45)) + 
    ggplot2::labs(title = 'Late-run proportion of optimal responses',
                  x='')

ggplot2::ggplot(lowfi_df_dev,
                ggplot2::aes(x = factor(condition, levels = condition_labels), y = bin_start_to_4)) + 
    ggplot2::geom_boxplot(width = .5) + 
    ggplot2::facet_grid(~sample) + 
    ggplot2::theme(axis.text.x = element_text(angle = 360 - 45)) + 
    ggplot2::labs(title = 'Early-run proportion of optimal responses',
                  x = '')
```
...
```{r}
psych::corr.test(cbind(splt_fsmi$fsmi_qs_67, splt_fsmi$dominance_prestige_18, splt_fsmi_ksrq_scored[,-5]), use = 'pairwise.complete.obs', method = 'spearman', adjust = 'none')
```

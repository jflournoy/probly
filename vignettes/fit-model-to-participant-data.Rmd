...
logistic_saves_dir <- '/data/jflournoy/split/probly/logistic/'
if(!dir.exists(logistic_saves_dir)) dir.create(logistic_saves_dir)
...
splt_no_na_dev <- dplyr::left_join(splt_no_na,
                                   splt_dev_and_demog, by = c('id' = 'SID', 'sample'))

splt_no_na_dev$trial_index_c0_s <- 
    (splt_no_na_dev$trial_index - max(splt_no_na_dev$trial_index)) /
    sd(splt_no_na_dev$trial_index)

splt_no_na_dev$condition_trial_index_c0_s <- 
    (splt_no_na_dev$condition_trial_index - max(splt_no_na_dev$condition_trial_index)) /
    sd(splt_no_na_dev$condition_trial_index)

null_id_only_m <- lme4::glmer(press_opt ~ 1 + (1 | id), 
                              family = 'binomial',
                              data = splt_no_na, REML = F)
null_m <- lme4::glmer(press_opt ~ 1 + (1 | sample/id), 
                      family = 'binomial',
                      data = splt_no_na, REML = F)
null_stim_rx_m <- lme4::glmer(press_opt ~ 1 + (1 | sample/id) + (1 | stim_image), 
                              family = 'binomial', 
                              data = splt_no_na, REML = F)
summary(null_m)
...
null_id_only_bm_f <- future::future(
    probly::CachedFit(
        {
            afit <- brms::brm(press_opt ~ 1 + (1 | id) + (1 | stim_image),
                              family = 'bernoulli',
                              data = splt_no_na, 
                              cores = 4, chains = 4, 
        ...
        rds_filename = file.path(logistic_saves_dir, 'null_id_only_bm.rds')))

null_bm_f <- future::future(
    probly::CachedFit(
        {
            afit <- brms::brm(press_opt ~ 1 + (1 | sample/id) + (1 | stim_image),
                              family = 'bernoulli',
                              data = splt_no_na, 
                              cores = 4, chains = 4, 
                              iter = 1500, warmup = 1000,
        ...
        rds_filename = file.path(logistic_saves_dir, 'null_bm.rds')))

time_m_f <- future::future(
    probly::CachedFit(
        lme4::glmer(press_opt ~ 1 + trial_index_c0_s + 
                        (1 + trial_index_c0_s | sample:id) + 
                        (1 | sample) + 
                        (1 + trial_index_c0_s | stim_image), 
               family = 'binomial',
               data = splt_no_na_dev, REML = F),
        rds_filename = file.path(logistic_saves_dir, 'time_m.rds')))

condition_m_f <- future::future(
    probly::CachedFit(
        lme4::glmer(press_opt ~ 1 + condition + 
                        (1 + condition | sample:id) + 
                        (1 | sample) + 
                        (1 + condition | stim_image), 
               family = 'binomial',
               data = splt_no_na, REML = F),
        rds_filename = file.path(logistic_saves_dir, 'condition_m.rds')))


resolved(null_id_only_bm_f)
resolved(null_bm_f)
resolved(time_m_f)
resolved(condition_m_f)

null_id_only_bm <- value(null_id_only_bm_f)
null_bm <- value(null_bm_f)
time_m <- value(time_m_f)
condition_m <- value(condition_m_f)

summary(value(null_id_only_bm_f))
add_ic(null_id_only_bm, ic = 'loo')

summary(value(null_bm_f))
add_ic(null_bm, ic = 'loo')

summary(value(time_m_f))
arm::invlogit(fixef(time_m)[1] + c(0, fixef(time_m)[2]*-3.45))

summary(value(condition_m_f))

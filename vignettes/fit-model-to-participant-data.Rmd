...
some_cors <- unlist(lapply(seq_len(dim(stanfit_2_level_no_b_betas)[1]),
       function(i){
           cor(arm::logit(stanfit_2_level_betas[i, betanames[(313*3+1):(2*3*313)]]),
               arm::logit(stanfit_2_level_no_b_betas[i, betanames[(313*3+1):(2*3*313)]]))
       }))
mean(some_cors)
quantile(some_cors, probs = c(.025, .5, .975))
hist(some_cors)
...
GGally::ggpairs(as.data.frame(betas_xi_means))
GGally::ggpairs(as.data.frame(betas_ep_means))
GGally::ggpairs(as.data.frame(betas_rho_means))
```

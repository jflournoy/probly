...
```{r echo=F, eval=F}
usethis::use_data(splt_lowfi_outcomes)
```
...
# #
# #Models should be re-estimated using the current version of brms before running
# #this section.
# plot(conditional_effects(age_model), ask = F)
# plot(conditional_effects(age_model), ask = F)
# plot(conditional_effects(fsmi_model_bf), ask = F)
# plot(conditional_effects(age_model_bf), ask = F)
```
...
Models labeled "repar_exp" are 3-level models, whereas those labeled "2_level" are 2-level models.

```{r}
stanfit_2_level <- readRDS(file.path(data_dir,'splt-looser-rl_2_level-1545567.RDS'))
stanfit_list <- lapply(
...
loo_comp <- loo::loo_compare(x = stan_loos)
knitr::kable(loo_comp)
```

If we can trust these LOO estimates (it would be better to do leave-one-observation-out rather than leave-one-participant-out), it looks like the three level structure is probably unnecessary but that the full parameterization is helpful.
...
beta_names_list <- lapply(
    ...
    function(par){
        grep(paste0('^', par, '\\[\\d+,\\d+\\]'),
             names(mean_pars$`splt-looser-rl_2_level-1545567.RDS`),
             value = T)
    })
...
```{r fig.width=20,fig.height=20, echo=FALSE, message=FALSE}
print(par_mean_cor_plot)
```

```{r corparplot, fig.width=5.875, fig.height=4, include=F, echo=FALSE, message=FALSE}
...
#get just betas 
stanfit_2_level_betas <- stanfit_list$`splt-looser-rl_2_level-1545567.RDS`$parameters[unlist(beta_names_list[1:4])]
...
splt_rl_betas_sum <- splt_rl_betas %>%
    ...
    dplyr::mutate(mean_par_old = mean_par, 
                  mean_par = case_when(parameter == 'ep_prm' ~ qnorm(mean_par),
...
```{r echo = F, eval = F}
usethis::use_data(splt_rl_betas, overwrite = TRUE)
```
...
modeled_splt_data <- readRDS(file.path(data_dir, 'splt-looser-data--1545416.RDS'))
...
This will tell you the relative proportion of variability across the three conditions.
...
```{r}
varprop_df <- as.data.frame(rbind(var_prop_ep, var_prop_xi, var_prop_rho, var_prop_b))
names(varprop_df) <- c('ht', 'dl', 'pu')
varprop_df$tile <- rownames(var_prop_ep)
varprop_df$par <- rep(c('epsilon', 'xi', 'rho', 'b'), each = 3)
...
splt_data_used <- readRDS('/data/jflournoy/split/probly/splt-looser-data--1545416.RDS')
...
splt_data_used_pR_block_sum_plot <- ggplot2::ggplot(dplyr::ungroup(splt_data_used_pR_block_sum),
...
stanfit_orig <- readRDS('/data/jflournoy/split/probly/splt-looser-rl_2_level-1545567.RDS')
stanfit <- stanfit_orig
rstan::stan_rhat(stanfit_orig)
rstan::stan_diag(stanfit_orig, information = 'stepsize')
rstan::stan_diag(stanfit_orig, information = 'treedepth')
rstan::stan_diag(stanfit_orig, information = 'divergence')

print(stanfit_orig, par = grep('L_Omega.*', names(stanfit_orig), value  = T))
print(stanfit_orig, par = grep('tau.*', names(stanfit_orig), value  = T))

mcmc_trace(as.array(stanfit_orig), par = 'beta_ep_prm[181,2]')

for(i in 1:length(munames)){
    rstan::stan_par(stanfit_orig, par = munames[i])
}
rstan::stan_rhat(stanfit_orig, pars = munames)
rstan::stan_ess(stanfit_orig, pars = munames)
pairs(stanfit_orig, pars = munames, condition = 'accept_stat__')
...
pairs(stanfit_orig, pars = munames)
pairs(stanfit_orig, pars = taunames)
```

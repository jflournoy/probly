...
```{r}
library(probly)
data('splt_clean', 'metacog_clean')

knitr::kable(
    dplyr::summarize(
        dplyr::group_by(
            dplyr::distinct(splt_clean, id, dir, filename, sample),
            sample),
        n = n()))
```
...
splt_averages_over_samples <- split_clean %>%
    dplyr::filter(!is.na(sample), sample != 'TDS3') %>%
...
ggplot2::ggplot(
    dplyr::filter(metacog_clean, !is.na(sample), sample != 'TDS3'),

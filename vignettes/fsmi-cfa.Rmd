...
uppsp_fsmi_partners_nullnull_model <- paste(
    sensseek_urgency_cfa,
     'fsmi_mate =~ fsmi_qs_55 + fsmi_qs_56 + fsmi_qs_57 + fsmi_qs_58 + fsmi_qs_59 + fsmi_qs_60
',
...
if(!file.exists(fsmi_num_partners_modelsFN)){
    ...
    uppsp_fsmi_partners_nullnull_sem <- lavaan::lavaan(
        model = uppsp_fsmi_partners_nullnull_model, 
        data = splt_fsmi_devdemog[splt_fsmi_devdemog$partnered == 0 &
                                      grepl('yads', splt_fsmi_devdemog$sample),],
        missing = 'listwise', estimator = 'WLSMV',
        int.ov.free = TRUE, int.lv.free = FALSE, 
    ...
    uppsp_fsmi_partners_lrt <- lavaan::lavTestLRT(uppsp_fsmi_partners_sem,
                                                  ...
                                                  uppsp_fsmi_partners_nullnull_sem)
...
anewdata <- 
lavaan::lavPredict(ksrq_num_partners_sem,)
...
```{r}
knitr::kable(
    dplyr::filter(standardizedsolution(ksrq_num_partners_sem, ci = T), op == '~'),
    digits = 2,
    caption = paste0(
...
knitr::kable(
    dplyr::filter(standardizedsolution(ksrq_noadmrtn_num_partners_sem, ci = T), op == '~'),
    digits = 2,
    caption = paste0(
        'K-SRQ Predicting number of partners among unpartnered college students; N obs. = ',
        lavaan::nobs(ksrq_num_partners_sem)))
...
knitr::kable(
    dplyr::filter(standardizedsolution(ksrq_num_partners_all_sem, ci = T), op == '~'),
    digits = 2,
    caption = paste0(
...
knitr::kable(
    dplyr::filter(standardizedsolution(ksrq_noadmrtn_num_partners_all_sem, ci = T), op == '~'),
    digits = 2,
    caption = paste0(
        'K-SRQ Predicting number of partners among all adolescent and college participants with SES responses; N obs. = ',
        lavaan::nobs(ksrq_num_partners_all_sem)))
...
knitr::kable(
    dplyr::filter(standardizedsolution(uppsp_ksrq_num_partners_sem, ci = T), op == '~'),
    digits = 2,
    caption = paste0(
        'K-SRQ Predicting number of partners among unpartnered college students; N obs. = ',
...
knitr::kable(
    dplyr::filter(standardizedsolution(uppsp_ksrq_nosr_num_partners_sem, ci = T), op == '~'),
    digits = 2,
    caption = paste0(
        'K-SRQ Predicting number of partners among unpartnered college students; N obs. = ',
...
knitr::kable(
    dplyr::filter(standardizedsolution(uppsp_ksrq_num_partners_all__sem, ci = T), op == '~'),
    digits = 2,
    caption = paste0(
        'K-SRQ Predicting number of partners among all adolescent and college participants with SES responses; N obs. = ',
...
knitr::kable(
    dplyr::filter(standardizedsolution(uppsp_ksrq_nosr_num_partners_all__sem, ci = T), op == '~'),
    digits = 2,
    caption = paste0(
        'K-SRQ Predicting number of partners among all adolescent and college participants with SES responses; N obs. = ',
...
### SPLT sexual relationship

```{r fig.width=4, fig.height=4}
splt_ep_num_partners_model <- paste(rl_var_names_facts[[1]], 
                                 'ordered_num_partner ~ ep_prm_DtngLHngT_fac + gender_c', sep = '\n')
splt_ep_num_partners_all_model <- paste(rl_var_names_facts[[1]], 
                                 'ordered_num_partner ~ ep_prm_DtngLHngT_fac + gender_c + age_c', sep = '\n')

splt_ep_bl_num_partners_model <- paste(rl_var_names_facts[[1]], 
                                 'ordered_num_partner ~ ep_prm_HngryThrsty_fac + gender_c', sep = '\n')
splt_ep_bl_num_partners_all_model <- paste(rl_var_names_facts[[1]], 
                                 'ordered_num_partner ~ ep_prm_HngryThrsty_fac + gender_c + age_c', sep = '\n')

splt_ep_num_partners_sem <- lavaan::lavaan(
    model = splt_ep_num_partners_model, 
...
splt_ep_num_partners_all_sem <- lavaan::lavaan(
    model = splt_ep_num_partners_all_model, 
    data = splt_fsmi_devdemog_betas,
    missing = 'listwise', estimator = 'WLSMV',
    int.ov.free = TRUE, int.lv.free = FALSE, 
    auto.fix.first = T, std.lv = F, std.ov = F,
...
splt_ep_bl_num_partners_sem <- lavaan::lavaan(
    model = splt_ep_bl_num_partners_model, 
    data = splt_fsmi_devdemog_betas[splt_fsmi_devdemog_betas$partnered == 0 &
                                  grepl('yads', splt_fsmi_devdemog_betas$sample),],
    missing = 'listwise', estimator = 'WLSMV',
    int.ov.free = TRUE, int.lv.free = FALSE, 
...
splt_ep_bl_num_partners_all_sem <- lavaan::lavaan(
    model = splt_ep_bl_num_partners_all_model, 
    data = splt_fsmi_devdemog_betas,
    missing = 'listwise', estimator = 'WLSMV',
    int.ov.free = TRUE, int.lv.free = FALSE, 
    auto.fix.first = T, std.lv = F, std.ov = F,
...
plot_lav_factor_pred(lavfit = fsmi_sex_feq_sem,
    ...
    ggplot2::labs(color = 'Partnered', 
                  ...
                  y = 'Number of sexual encounters,\npast 6 mo.') + 
    ggplot2::geom_smooth(method = 'lm', 
                         formula = y ~ x,
                         ggplot2::aes(y = as.numeric(dv_dat)), alpha = .2)
...
uppsp_ksrq_protection_model <- paste(
    upps_cfa_model,
    'k_srq_sexual_relationships =~ K_SRQ_9 + K_SRQ_13 + K_SRQ_20',
    'continuous_use_protection ~ neg_urgency + premeditation + perseverance + sensation_seeking + pos_urgency + k_srq_sexual_relationships + gender_c + age_c',
    sep = '\n')

uppsp_ksrq_protection_null_model <- paste(
    upps_cfa_model,
     'k_srq_sexual_relationships =~ K_SRQ_9 + K_SRQ_13 + K_SRQ_20',
     'continuous_use_protection ~ neg_urgency + premeditation + perseverance + sensation_seeking + pos_urgency + 0*k_srq_sexual_relationships + gender_c + age_c',
     sep = '\n')

uppsp_ksrq_protection_nullnull_model <- paste(
    upps_cfa_model,
     'k_srq_sexual_relationships =~ K_SRQ_9 + K_SRQ_13 + K_SRQ_20',
     'continuous_use_protection ~ 0*neg_urgency + 0*premeditation + 0*perseverance + 0*sensation_seeking + 0*pos_urgency + 0*k_srq_sexual_relationships + gender_c + age_c',
     sep = '\n')

ksrq_protection_adol_modelsFN <- file.path(data_dir, 'ksrq_protection_adol_models.rda')
if(!file.exists(ksrq_protection_adol_modelsFN)){
    ...
    uppsp_ksrq_protection_adol_sem <- lavaan::lavaan(
        model = uppsp_ksrq_protection_model, 
        data = splt_fsmi_devdemog,
        missing = 'fiml', estimator = 'MLR',
        int.ov.free = TRUE, int.lv.free = FALSE, 
        auto.fix.first = T, std.lv = F, 
    ...
    uppsp_ksrq_protection_adol_null_sem <- lavaan::lavaan(
        model = uppsp_ksrq_protection_null_model, 
        data = splt_fsmi_devdemog,
        missing = 'fiml', estimator = 'MLR',
        int.ov.free = TRUE, int.lv.free = FALSE, 
        auto.fix.first = T, std.lv = F, 
    ...
    uppsp_ksrq_protection_adol_nullnull_sem <- lavaan::lavaan(
        model = uppsp_ksrq_protection_nullnull_model, 
        data = splt_fsmi_devdemog,
        missing = 'fiml', estimator = 'MLR',
        int.ov.free = TRUE, int.lv.free = FALSE, 
        auto.fix.first = T, std.lv = F, 
    ...
    save(ksrq_protection_adol_sem,
         ksrq_protection_adol_null_sem,
         uppsp_ksrq_protection_adol_sem,
         uppsp_ksrq_protection_adol_null_sem,
         uppsp_ksrq_protection_adol_nullnull_sem,
         file = ksrq_protection_adol_modelsFN)
} else {
    load(ksrq_protection_adol_modelsFN)
}
lavaan::lavTestLRT(uppsp_ksrq_protection_adol_sem,
                   uppsp_ksrq_protection_adol_null_sem,
                   uppsp_ksrq_protection_adol_nullnull_sem)
lavaan::lavTestLRT(ksrq_protection_adol_null_sem,
...
## alcohol_days_of_lst30

## alcohol_5drnks_lst30

...
splt_fsmi_devdemog$ordered_num_partner <- ordered(
    ifelse(splt_fsmi_devdemog$SES_13 < 4, splt_fsmi_devdemog$SES_13, 4),
    levels = 0:4,
    labels = c(0:3, '4 or more'))

ggplot2::ggplot(data = splt_fsmi_devdemog[!is.na(splt_fsmi_devdemog$ordered_sex_freq) & !is.na(splt_fsmi_devdemog$partnered),],
                ggplot2::aes(x = ordered_sex_freq,
                             fill = as.factor(partnered))) + 
    ggplot2::geom_histogram(stat = 'count',
                            position = ggplot2::position_dodge(), alpha = .8)
ggplot2::ggplot(data = splt_fsmi_devdemog[!is.na(splt_fsmi_devdemog$ordered_num_partner) & !is.na(splt_fsmi_devdemog$partnered),],
...
fsmi_num_partners_model <- paste(fsmi_cfa_matestat_model, 
                                 'ordered_num_partner ~ fsmi_mate', sep = '\n')

fsmi_num_partners_sem <- lavaan::lavaan(model = fsmi_num_partners_model, 
                                        ...
                                        auto.cov.lv.x = TRUE, auto.th = TRUE, 
                                        auto.delta = TRUE, auto.cov.y = TRUE)
summary(fsmi_num_partners_sem, stand = T)

ksrq_num_partners_model <- paste(ksrq_3factor_model, 
                                 'ordered_num_partner ~ ML2', sep = '\n')

ksrq_num_partners_sem <- lavaan::lavaan(model = ksrq_num_partners_model, 
                                        data = splt_fsmi_devdemog[splt_fsmi_devdemog$partnered == 0,],
                                        missing = 'listwise', estimator = 'WLSMV',
                                        int.ov.free = TRUE, int.lv.free = FALSE, 
                                        ...
                                        auto.cov.lv.x = TRUE, auto.th = TRUE, 
                                        auto.delta = TRUE, auto.cov.y = TRUE)
summary(ksrq_num_partners_sem, stand = T)

fsmi_sex_feq_model <- paste(fsmi_cfa_matestat_model, 
                                 'ordered_sex_freq ~ fsmi_mate', sep = '\n')
...
fsmi_sex_feq_sem <- lavaan::lavaan(model = fsmi_sex_feq_model, 
                                        ...
                                        auto.delta = TRUE, auto.cov.y = TRUE,
                                   group = 'partnered')
summary(fsmi_sex_feq_sem, stand = T)

ksrq_sex_freq_model <- paste(ksrq_3factor_model, 
                                 'ordered_sex_freq ~ ML2', sep = '\n')

...
splt_rl_betas_w <- tidyr::spread(
    ...
    parcond, mean)
splt_fsmi_devdemog_yads_betas <- left_join(splt_fsmi_devdemog_yads,
                                           splt_rl_betas_w, 
                                           by = c('SID' = 'id'))

rl_var_names <- names(dplyr::select(splt_rl_betas_w, -id))
rl_var_names_facts <- paste(paste0(paste0(rl_var_names, '_fac'),
                                   '=~', rl_var_names),
                            collapse = '\n')
rl_var_names_cor <- paste0(paste(rl_var_names, collapse = ' + '), 
                           '~~', 
                           paste(all_scale_latent_varnames, collapse = ' + '))

all_scales_college_cor_load_splt <- lavaan::lavaan(
    model = paste(fsmi_cfa_matestat_model, dnp_cfa_model,
    ...
    group.equal = c('loadings',
                    'lv.variances',
                    'lv.covariances'),
    verbose = T, optim.method = 'BFGS')
# control=list(init_nelder_mead=TRUE), verbose = TRUE)
#optim.method = 'optim'
# all_scales_college_cor_load_splt_std <- standardizedsolution(all_scales_college_cor_load_splt, ci = T)
```

```{r fig.width=3.25, fig.height=1}
ggplot2::theme_set(ggplot2::theme_minimal())
ggplot2::ggplot(
    data.frame(cors = as.numeric(inspect(all_scales_college_cor_load_splt, 
                                         'cor.lv')[[1]][14:(14+4*5-1), 1:13])),
    ggplot2::aes(x = cors)) + 
    ggplot2::geom_vline(xintercept = 0) + 
    ggplot2::geom_histogram(binwidth = .01, alpha = .8) +
    ggplot2::scale_x_continuous(breaks = c(-1, -.1, .1, 1)) + 
...
all_scales_college_cor_par <- standardizedSolution(all_scales_college_partnered_cor_load, ci = TRUE)

all_scales_college_cor_fit <- fitmeasures(all_scales_college_partnered_cor_load, c('rmsea', 'mfi'))
...
adol_cor_sensitivity_abs_max <- max(abs(inspect(all_scales_adol_cor_load, 'cor.lv')[['0']] - 
    inspect(all_scales_adol_cor, 'cor.lv')[['0']]))
```

```{r}
all_scales_adol_par <- standardizedSolution(all_scales_adol, ci = TRUE)
all_scales_adol_cor_par <- standardizedSolution(all_scales_adol_cor, ci = TRUE)
...
all_scales_adol_pardf <- dplyr::mutate(
    tidyr::spread(
        tidyr::unite(
            tidyr::gather(
                dplyr::mutate_at(
                    dplyr::filter(all_scales_adol_pardf_l, group == 3),
                    dplyr::vars(lhs,rhs),
                    dplyr::funs(factor(., levels = all_scale_latent_varnames, 
                                       labels = all_scale_latent_varnames))),
                par, value, est.std, ci.upper, ci.lower, z), 
...
all_scales_adol_plot <- ggplot2::ggplot(
    dplyr::mutate_at(all_scales_adol_pardf_l,
                     dplyr::vars(lhs,rhs),
                     funs(factor(., levels = all_scale_latent_varnames, 
                                 labels = all_scale_latent_varnames))),
    ggplot2::aes(x = factor(group, levels = 1:3, labels = c('F', 'M', '-')), 

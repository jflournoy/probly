...
ksrq_mi_model <- 'k_srq_admiration =~ K_SRQ_1 + K_SRQ_7 + K_SRQ_11 + K_SRQ_18
k_srq_sexual_relationships =~ K_SRQ_9 + K_SRQ_13 + K_SRQ_20'

ksrq_mi_unconstrained <- lavaan::lavaan(
    model = ksrq_mi_model, 
...
ksrq_mi_metric <- lavaan::lavaan(
    ...
    auto.cov.lv.x = TRUE, auto.th = TRUE, 
    auto.delta = TRUE, auto.cov.y = TRUE,
    group = 'samplegroup', group.equal = c('loadings', 'intercepts'))

ksrq_mi_fitmeasures <- unlist(lapply(
    list(ksrq_mi_unconstrained = ksrq_mi_unconstrained,
         ksrq_mi_metric = ksrq_mi_metric),
    lavaan::fitmeasures, fit.measures = 'mfi'))
diff(ksrq_mi_fitmeasures)

ksrq_mi_extfitmeasures <- unlist(lapply(
    list(ksrq_mi_unconstrained = ksrq_mi_unconstrained,
         ksrq_mi_metric = ksrq_mi_metric),
    semTools::moreFitIndices, fit.measures = c('gammaHat', 'adjGammaHat')))
ksrq_mi_extfitmeasures[3:4]-ksrq_mi_extfitmeasures[1:2]
```

This small difference in the MFI Indicates that we do not need to reject the null hypothesis of measurement invariance. However, scalar/strong invariance is rejected so comparing means or variances of computed scores is possibly problematic.
...
ksrq_age_ints <- inspect(ksrq_age_sem, 'mean.lv')
ksrq_age_betas <- inspect(ksrq_age_sem, 'coef')$beta[1:4, c(5,7,6,8,9)]
...
participant_measurements <- lavaan::lavPredict(ksrq_age_sem)
ksrq_age_model_data <- as.data.frame(lavaan::inspect(ksrq_age_sem, 'data'))
names(ksrq_age_model_data) <- lavaan::lavNames(ksrq_age_sem)
...
ksrq_age_table <- knitr::kable(
    dplyr::filter(parameterestimates(ksrq_age_sem, stan = T), op == '~'),
    digits = 2,
    caption = paste0('K-SRQ and age; N obs. = ',
                     lavaan::nobs(ksrq_age_sem)))
ksrq_age_table
ksrq_age_pred_plot
...
ksrq_pds_sem <- lavaan::lavaan(
    model = ksrq_pds_model, 
    data = pds_mod_data,
    missing = 'fiml', estimator = 'MLR',
    int.ov.free = TRUE, int.lv.free = FALSE, 
    auto.fix.first = T, std.lv = F, 
...
ksrq_pds_lin_sem <- lavaan::lavaan(
    model = ksrq_pds_model_lin, 
    data = pds_mod_data,
    missing = 'fiml', estimator = 'MLR',
    int.ov.free = TRUE, int.lv.free = FALSE, 
    auto.fix.first = T, std.lv = F, 
...
anova(ksrq_pds_lin_sem, ksrq_pds_sem)
```

```{r}
ksrq_pds_ints <- inspect(ksrq_pds_sem, 'mean.lv')
ksrq_pds_betas <- inspect(ksrq_pds_sem, 'coef')$beta[1:4, c(5,7,6,8,9)]

pds_gender_pred_df <- as.data.frame(expand.grid(pds = seq(1,4,.25), gender_c = c(-.5, .5),
                                                sample = na.omit(unique(pds_mod_data$sample))))
pds_gender_pred_df$pds_c <- pds_gender_pred_df$pds - 3.0
...
ksrq_pds_pred_factor_scores <- do.call(cbind, lapply(seq_along(ksrq_pds_ints), function(i){
    y <- pds_gender_pred_mat %*% as.vector(c(ksrq_pds_ints[[i]], ksrq_pds_betas[i,]))
    dimnames(y)[2] <- names(ksrq_pds_ints)[[i]]
    y
}))

participant_measurements_pds <- lavaan::lavPredict(ksrq_pds_sem)
ksrq_pds_model_data <- as.data.frame(lavaan::inspect(ksrq_pds_sem, 'data'))
names(ksrq_pds_model_data) <- lavaan::lavNames(ksrq_pds_sem)
ksrq_pds_model_data <- cbind(ksrq_pds_model_data, participant_measurements_pds)
ksrq_pds_model_data$sample <- pds_mod_data$sample
ksrq_pds_model_data_l <- tidyr::gather(
    dplyr::select(ksrq_pds_model_data, pds_c:gender_c, k_srq_admiration:k_srq_sociability, sample),
    factor,
    value,
    -gender_c, -pds_c, -pds_c2, -sample)
ksrq_pds_model_data_l$pds <- ksrq_pds_model_data_l$pds_c + 3.0
...
pds_gender_pred_df <- tidyr::gather(
    cbind(pds_gender_pred_df, ksrq_pds_pred_factor_scores),
    factor,
    value,
...
ksrq_pds_pred_plot <- ggplot2::ggplot(
    pds_gender_pred_df,
    ggplot2::aes(x = pds, y = value, group = gender_c, color = factor(gender_c))) + 
    ggplot2::geom_point(data = ksrq_pds_model_data_l,
                        alpha = .5) +
    ggplot2::geom_line(data = ksrq_pds_model_data_l, 
                       stat = 'smooth') +
    ggplot2::geom_line() +
    ...
    ggplot2::coord_cartesian(xlim = c(min(ksrq_pds_model_data_l$pds, na.rm = T), 
                             ...
                             ylim = c(-4.5, 3)) + 
    ggplot2::scale_color_manual(name = 'Gender', breaks = c(-.5, .5), values = c('#1f78b4', '#33a02c'),
                                labels = c('Male', 'Female')) + 
    ggplot2::theme_minimal()


ksrq_pds_table <- knitr::kable(
    dplyr::filter(parameterestimates(ksrq_pds_sem, stan = T), op == '~'),
    digits = 2,
    caption = paste0('K-SRQ and age; N obs. = ',
                     lavaan::nobs(ksrq_pds_sem)))
ksrq_pds_table
ksrq_pds_pred_plot
```
...
uppsp_cfa_mi_unconstrained <- lavaan::lavaan(model = uppsp_cfa_model, data = splt_fsmi_devdemog,
                            missing = 'fiml', estimator = 'MLR',
                            int.ov.free = TRUE, int.lv.free = FALSE, 
                            auto.fix.first = FALSE, std.lv = TRUE, 
                            auto.fix.single = TRUE, auto.var = TRUE, 
...
uppsp_cfa_mi_metric <- lavaan::lavaan(model = uppsp_cfa_model, data = splt_fsmi_devdemog,
                            missing = 'fiml', estimator = 'MLR',
                            int.ov.free = TRUE, int.lv.free = FALSE, 
                            auto.fix.first = FALSE, std.lv = TRUE, 
                            auto.fix.single = TRUE, auto.var = TRUE, 
...
(uppspfit <- lavaan::fitmeasures(uppsp_cfa, fit.measures = c('cfi', 'rmsea', 'mfi')))

uppsp_mi_fitmeasures <- unlist(lapply(
    list(uppsp_cfa_mi_unconstrained = uppsp_cfa_mi_unconstrained,
         uppsp_cfa_mi_metric = uppsp_cfa_mi_metric),
    lavaan::fitmeasures, fit.measures = 'mfi'))
diff(uppsp_mi_fitmeasures)

uppsp_mi_extfitmeasures <- unlist(lapply(
    list(uppsp_cfa_mi_unconstrained = uppsp_cfa_mi_unconstrained,
         uppsp_cfa_mi_metric = uppsp_cfa_mi_metric),
    semTools::moreFitIndices, fit.measures = c('gammaHat', 'adjGammaHat')))
uppsp_mi_extfitmeasures[3:4]-uppsp_mi_extfitmeasures[1:2]
```

The UPPS-P scale shows metric invariance, so we can look at covariance from latent factors.
...
uppsp_scored <- as.data.frame(
    psych::scoreItems(upps_key,
                      items = splt_fsmi_devdemog,
                      missing = TRUE,
                      impute = 'none',
                      min = 1, max = 4)$score)
uppsp_scored$age <- splt_fsmi_devdemog$age
uppsp_scored$gender_c <- splt_fsmi_devdemog$gender_c

ggplot2::theme_set(ggplot2::theme_minimal())
ggplot2::ggplot(
    tidyr::gather(uppsp_scored, scale, value, -age, -gender_c),
    ggplot2::aes(x = age, y = value, color = factor(gender_c), group = gender_c)
) + 
    ggplot2::geom_point(alpha = .5) + 
    ggplot2::geom_smooth(method = 'glm', formula = y ~ poly(x,2)) + 
    ggplot2::facet_wrap(~scale)
```

```{r}
uppsp_age_sem <- lavaan::lavaan(
    model = uppsp_age_model, 
    data = splt_fsmi_devdemog,
    missing = 'fiml', estimator = 'MLR',
    int.ov.free = TRUE, int.lv.free = FALSE, 
    auto.fix.first = T, std.lv = F, 
...
uppsp_age_ints <- inspect(uppsp_age_sem, 'mean.lv')
uppsp_age_betas <- inspect(uppsp_age_sem, 'coef')$beta[1:5, c(5,7,6,8,9)]
...
uppsp_pred_factor_scores <- do.call(cbind, lapply(seq_along(uppsp_age_ints), function(i){
    y <- age_gender_pred_mat %*% as.vector(c(uppsp_age_ints[[i]], uppsp_age_betas[i,]))
    dimnames(y)[2] <- names(uppsp_age_ints)[[i]]
    y
}))

uppsp_participant_measurements <- lavaan::lavPredict(uppsp_age_sem)
uppsp_age_model_data <- as.data.frame(lavaan::inspect(uppsp_age_sem, 'data'))
names(uppsp_age_model_data) <- lavaan::lavNames(uppsp_age_sem)
uppsp_age_model_data <- cbind(uppsp_age_model_data, uppsp_participant_measurements)
uppsp_age_model_data_l <- tidyr::gather(
    dplyr::select(uppsp_age_model_data, age_c:gender_c, neg_urgency:pos_urgency),
    factor,
    value,
    -gender_c, -age_c, -age_c2)
...
uppsp_age_gender_pred_df <- tidyr::gather(
    cbind(age_gender_pred_df, uppsp_pred_factor_scores),
    factor,
    value,
    -age, -gender_c, -age_c, -age_c2)
...
uppsp_age_pred_plot <- ggplot2::ggplot(
    ...
    ggplot2::aes(x = age, y = value, group = gender_c, color = factor(gender_c))) + 
    ggplot2::geom_point(data = uppsp_age_model_data_l,
                        alpha = .5) +
    ggplot2::geom_line(data = uppsp_age_model_data_l, 
                       stat = 'smooth') +
    ggplot2::geom_line() + 
    ggplot2::facet_wrap(~factor) + 
...
uppsp_age_table <- knitr::kable(
    dplyr::filter(parameterestimates(uppsp_age_sem, stan = T), op == '~'),
    digits = 2,
    caption = paste0('K-SRQ and age; N obs. = ',
                     lavaan::nobs(uppsp_age_sem)))
uppsp_age_table
uppsp_age_pred_plot
```

...
get_chisq_tests <- function(an_sem, verbose = T, lrt_idxs = c(5,6)){
    ...
    thesign <- ifelse(
        sign(dplyr::filter(
            parameterestimates(an_sem[[dimnames(an_sem[[lrt_idxs[1]]])[[1]][[1]]]]), 
            op == '~', 
            !rhs %in% c('gender_c', 'age_c'))[['est']]) == -1, 
        '-', '+')
    ...
    thesign_all <- ifelse(
        sign(dplyr::filter(
            parameterestimates(an_sem[[dimnames(an_sem[[lrt_idxs[2]]])[[1]][[1]]]]), 
            op == '~', 
            !rhs %in% c('gender_c', 'age_c'))[['est']]) == -1, 
        '-', '+')
...
```{r}
do_alpha_df <- rbind(expand.grid(sample = c('yads', 'yads_online'), 
                           key = c(fsmi_key[c('fsmi_mate', 'fsmi_stat')], dnp_key)),
                         expand.grid(sample = c('yads', 'yads_online', 'TDS1', 'TDS2'), 
                           key = c(ksqr_key[c(1,3,5,6)], upps_key)))
alpha_thing <- bind_rows(lapply(1:nrow(do_alpha_df),
      function(i){
          thing <- psych::alpha(
              x = splt_fsmi_devdemog[splt_fsmi_devdemog$sample == do_alpha_df$sample[i],], 
              keys = do_alpha_df[i,2])
          thing_df <- as.data.frame(thing$total)
          thing_df$sample <- do_alpha_df$sample[i]
          thing_df$keyname <- names(do_alpha_df[i,2])
          return(thing_df)
      }))

scale_labels <- c('fsmi_mate' = '(F) Mate-seeking',
                  'fsmi_stat' = '(F) Status',
                  'k_srq_admiration' = '(K) Admiration',
                  'k_srq_passivity' = '(K) Passivity',
                  'k_srq_sexual_relationships' = '(K) Sexual Rel.',
                  'k_srq_sociability' = '(K) Sociability',
                  'dominance_score' = '(D) Dominance',
                  'prestige_score' = '(D) Presitige',
                  'neg_urgency' = '(U) -Urgency',
                  'premeditation' = '(U) Premeditation',
                  'perseverance' = '(U) Perseverance',
                  'sensation_seeking' = '(U) Sensation S.',
                  'pos_urgency' = '(U) +Urgency')

scale_descriptives_table <- ungroup(
        mutate(group_by(
            arrange(
                mutate(
                    select(alpha_thing,keyname, sample, raw_alpha, `G6(smc)`, mean, sd), 
                    sample = factor(as.character(sample),
                                    levels = names(sample_labels_short),
                                    labels = sample_labels_short),
                    keyname = factor(as.character(keyname),
                                     levels = names(scale_labels),
                                     labels = scale_labels),
                    keynamegrp = keyname),
                keynamegrp, sample),
            keynamegrp), keyname = c(as.character(keyname)[1], rep('',n()-1))))

scale_descriptives_table_col <- select(
    filter(scale_descriptives_table, 
           grepl('\\((F|D)\\)', keynamegrp)),
    -keynamegrp)
scale_descriptives_table_comb_ksrq <- select(
    filter(scale_descriptives_table, 
           grepl('\\((K)\\)', keynamegrp)),
    -keynamegrp)

scale_descriptives_table_comb_uppsp <- select(
    filter(scale_descriptives_table, 
           grepl('\\((U)\\)', keynamegrp)),
    -keynamegrp)

knitr::kable(
    scale_descriptives_table_col,
    col.names = c('Scale name', 'Sample', '$\\alpha$', '$\\lambda\\text{-6}$', 'Mean', 'SD'),
    digits = 2)

knitr::kable(
    scale_descriptives_table_comb_ksrq,
    digits = 2)

knitr::kable(
    scale_descriptives_table_comb_uppsp,
    digits = 2)
```
...
### LTR and Dating/Looking params

```{r}
PairedData::yuen.t.test(ep_prm_DtngLHngT ~ factor(partnered), data = splt_fsmi_devdemog_betas[!is.na(splt_fsmi_devdemog_betas$partnered) & grepl('yads',splt_fsmi_devdemog$sample) & !is.na(splt_fsmi_devdemog_betas$ep_prm_DtngLHngT),])

PairedData::yuen.t.test(rho_prm_DtngLHngT ~ factor(partnered), data = splt_fsmi_devdemog_betas[!is.na(splt_fsmi_devdemog_betas$partnered) & grepl('yads',splt_fsmi_devdemog$sample) & !is.na(splt_fsmi_devdemog_betas$ep_prm_DtngLHngT),])

PairedData::yuen.t.test(xi_prm_DtngLHngT ~ factor(partnered), data = splt_fsmi_devdemog_betas[!is.na(splt_fsmi_devdemog_betas$partnered) & grepl('yads',splt_fsmi_devdemog$sample) & !is.na(splt_fsmi_devdemog_betas$ep_prm_DtngLHngT),])

PairedData::yuen.t.test(b_DtngLHngT ~ factor(partnered), data = splt_fsmi_devdemog_betas[!is.na(splt_fsmi_devdemog_betas$partnered) & grepl('yads',splt_fsmi_devdemog$sample) & !is.na(splt_fsmi_devdemog_betas$ep_prm_DtngLHngT),])

PairedData::yuen.t.test(bin_5_to_end_DtngLHngT ~ factor(partnered), data = splt_fsmi_devdemog_betas[!is.na(splt_fsmi_devdemog_betas$partnered) & grepl('yads',splt_fsmi_devdemog$sample) & !is.na(splt_fsmi_devdemog_betas$ep_prm_DtngLHngT),])

PairedData::yuen.t.test(bin_start_to_4_DtngLHngT ~ factor(partnered), data = splt_fsmi_devdemog_betas[!is.na(splt_fsmi_devdemog_betas$partnered) & grepl('yads',splt_fsmi_devdemog$sample) & !is.na(splt_fsmi_devdemog_betas$ep_prm_DtngLHngT),])

stats::t.test(ep_prm_DtngLHngT ~ factor(partnered), data = splt_fsmi_devdemog_betas[!is.na(splt_fsmi_devdemog_betas$partnered) & grepl('yads',splt_fsmi_devdemog$sample) & !is.na(splt_fsmi_devdemog_betas$ep_prm_DtngLHngT),])

stats::t.test(rho_prm_DtngLHngT ~ factor(partnered), data = splt_fsmi_devdemog_betas[!is.na(splt_fsmi_devdemog_betas$partnered) & grepl('yads',splt_fsmi_devdemog$sample) & !is.na(splt_fsmi_devdemog_betas$ep_prm_DtngLHngT),])

stats::t.test(xi_prm_DtngLHngT ~ factor(partnered), data = splt_fsmi_devdemog_betas[!is.na(splt_fsmi_devdemog_betas$partnered) & grepl('yads',splt_fsmi_devdemog$sample) & !is.na(splt_fsmi_devdemog_betas$ep_prm_DtngLHngT),])

stats::t.test(b_DtngLHngT ~ factor(partnered), data = splt_fsmi_devdemog_betas[!is.na(splt_fsmi_devdemog_betas$partnered) & grepl('yads',splt_fsmi_devdemog$sample) & !is.na(splt_fsmi_devdemog_betas$ep_prm_DtngLHngT),])

stats::t.test(bin_5_to_end_DtngLHngT ~ factor(partnered), data = splt_fsmi_devdemog_betas[!is.na(splt_fsmi_devdemog_betas$partnered) & grepl('yads',splt_fsmi_devdemog$sample) & !is.na(splt_fsmi_devdemog_betas$ep_prm_DtngLHngT),])

stats::t.test(bin_start_to_4_DtngLHngT ~ factor(partnered), data = splt_fsmi_devdemog_betas[!is.na(splt_fsmi_devdemog_betas$partnered) & grepl('yads',splt_fsmi_devdemog$sample) & !is.na(splt_fsmi_devdemog_betas$ep_prm_DtngLHngT),])
```
...
```{r}
splt_beh_model_lists <- c('splt_alc_5drnk_sems',
                          'splt_alc_days_sems',
                          'splt_num_partners_sems',
                          'splt_sex_freq_sems',
                          'splt_use_protection_sems')

splt_sems <- lapply(splt_beh_model_lists, function(a) {
    sems <- eval(parse(text = a))
    chisqs <- lapply(seq_along(sems), function(i){
        if(!'try-error' %in% class(sems[[i]])){
            return(c(splt_var = names(sems)[[i]],
                     get_chisq_tests(sems[[i]], verbose = T)))
        }
    })
    chisqs_df <- as.data.frame(do.call(rbind, chisqs))
    return(chisqs_df)
})
names(splt_sems) <- splt_beh_model_lists
...
splt_sems_df_4table <- dplyr::select(
    dplyr::arrange(
        dplyr::mutate(
            ...
            togroup = outcome_var),
        ordernum),
    togroup, contrast, outcome_var, splt_var, thesign, chisq, p, thesign_all, chisq_all, p_all)
